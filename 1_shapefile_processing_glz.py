...
def preprocessing_constructions(df):
    ...
    def random_constructions(row, construction_sets,
        ...
        if pd.isnull(row['AMD_glazing_ratio']):
            if row['use']=='Residential':
                ...
                row['AMD_glazing_ratio'] = int(10 * rd.randint(3,7))
...
def fix_glazing(df):
    previous_df = pd.read_csv(os.path.join(ROOT_DIR, 'amd_all.csv'))
    glazing_df = previous_df[['osgb','floor_no','AMD_glazing_ratio']]
    df = pd.merge(df, glazing_df, on=['osgb','floor_no'], suffixes=['_old', '_new'],sort=False)
    df.drop(columns=['AMD_glazing_ratio_old'],inplace=True)
    df.rename(columns={"AMD_glazing_ratio_new": "AMD_glazing_ratio"}, inplace=True)
    print(df.columns)
    return df
...
def main():
    ...
    df = pd.merge(shapes_df, df, on='osgb', sort=False)
    #print(df.head)
    df = expand_floors(df)
    df = combine_uses(df)
    #df = fix_glazing(df) #uncomment this to use previously generated glazing percentages for new runs
    output_df = preprocessing_constructions(df)
    print(output_df.head)
    output_df.to_csv(os.path.join(ROOT_DIR, 'amd_all.csv'), index=False)
